(() => {
    ...
    if (window.innerWidth > 549.98) {
        ...
        const maxLeaves = 15;
        let leavesCount = 0;
        const createLeaf = delay => {
            if (leavesCount >= maxLeaves) return;
            ...
            leaf.style.top = `-50px`;
            const animationDuration = Math.random() * (15 - 5) + 5;
            ...
            setTimeout((() => {
                leaf.remove();
                leavesCount--;
            }), (animationDuration + animationDelay) * 1e3);
        };
        const createLeavesSequentially = () => {
            let delay = 0;
            setInterval((() => {
                createLeaf(delay);
                delay += .5;
